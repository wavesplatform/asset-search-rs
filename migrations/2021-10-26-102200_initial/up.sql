...
CREATE INDEX IF NOT EXISTS assets_id_partial_idx
    ON assets USING gin (id gin_trgm_ops)
    WHERE superseded_by = 9223372036854775806 AND nft = false;
...
CREATE INDEX IF NOT EXISTS assets_name_partial_idx
    ON assets USING gin (name gin_trgm_ops)
    WHERE (superseded_by = 9223372036854775806) AND nft = false;

CREATE INDEX IF NOT EXISTS assets_name_tsvector_partial_idx
